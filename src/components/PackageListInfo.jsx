...
const PackageListInfo = (props) => {
    ...
    const [mockedMins, setMockedMins] = useState(null);   
    ...
    useEffect(() => {
      setMockedPrice(getRandomNumber(1, 100));
      setMockedMins(getRandomNumber(1, 30));
      setMockedAvailableSpots(getRandomNumber(2,10));
    },[])
        ...
        <Card style={{ cursor: "pointer" }} onClick={redirectParkingDetails}>
            <Card.Body>
                <Row style={{ padding: '14px' }}>
                    ...
                    <Col lg={7} style={{ lineHeight: '30px' }}>
                ...
                <Row style={{ display: showDetails ? 'flex' : 'none', lineHeight: '40px', padding: '15px' }}>
                    <Col lg={12}>
                        <div className="add-title"> {placeData.name}<span className="bold-text">({mockedMins} mins)</span> </div>
                        ...
                        <div className="bold-text"><img style={{ width: 20, height: 20 }} src="/images/star.png"></img> {placeData.rating} ({placeData.user_ratings_total} ratings)</div>
                         <div className="bold-text">{mockedAvailableSpots} Available Spots</div>
